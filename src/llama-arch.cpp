...
// Global fallback tensor name map for arches not in the per-arch LLM_TENSOR_NAMES below.
// This is the pre-Eagle3 flat map; arches added to spec-dec after the Eagle3 PR only need
// entries here if they use non-standard tensor names.
static const std::map<llm_tensor, const char *> LLM_TENSOR_NAMES_GLOBAL = {
...
static const std::map<llm_arch, std::map<llm_tensor, const char *>> LLM_TENSOR_NAMES = {
    ...
    },
    {
        LLM_ARCH_EAGLE3,
        {
            // Placeholders for tensors queried by generic quantization code (not present in Eagle3 models)
            { LLM_TENSOR_POS_EMBD,           "position_embd" },
            { LLM_TENSOR_TOKEN_TYPES,        "token_types" },
...
std::string LLM_TN_IMPL::str() const {
    ...
    if (arch_it != LLM_TENSOR_NAMES.end()) {
        auto tensor_it = arch_it->second.find(tensor);
        if (tensor_it != arch_it->second.end()) {
            fmt = tensor_it->second;
        }
    }

    if (!fmt) {
        auto global_it = LLM_TENSOR_NAMES_GLOBAL.find(tensor);
        if (global_it == LLM_TENSOR_NAMES_GLOBAL.end()) {
            GGML_ABORT("unknown tensor name for tensor id %d in arch %d", static_cast<int>(tensor), static_cast<int>(arch));
        }
        fmt = global_it->second;
    }
